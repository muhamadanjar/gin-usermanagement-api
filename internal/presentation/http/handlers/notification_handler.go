// internal/presentation/http/handlers/notification_handler.go
package handlers
...
func (h *NotificationHandler) SendToMe(c *gin.Context) {
	...
	response, err := h.notificationUseCase.SendToUser(principal.User.ID, req.Title, req.Body, req.Data)
	if err != nil {
		c.JSON(http.StatusInternalServerError, gin.H{"error": err.Error()})
		return
	}
